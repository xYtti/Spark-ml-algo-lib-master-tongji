...
class LDAConfig extends Serializable {
  ...
  @BeanProperty var lda: util.HashMap[String, util.HashMap[String, Object]] = _
}
...
object LDARunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val paramsMap: util.HashMap[String, Object] = config.lda.get(datasetCpuName)
      ...
      params.setSaveDataPath(s"hdfs:///tmp/ml/result/${params.algorithmName}/${datasetName}_${dataStructure}_${apiName}")
      ...
      Utils.saveYml[LDAParams](params, yaml)
...
class LDAKernel {
  ...
  def runPredictJob(spark: SparkSession, params: LDAParams, model: LDAModel): Double = {
    ...
    Utils.saveAndVerifyRes[LDAParams](params, res, sc)
    testData.unpersist()
    spark.close()
    res
  }
