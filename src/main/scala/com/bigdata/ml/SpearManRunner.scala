...
class SpearManConfig extends Serializable {
  ...
  @BeanProperty var spearman: util.HashMap[String, util.HashMap[String, Object]] = _
}
...
object SpearManRunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val paramsMap: util.HashMap[String, Object] = config.spearman.get(datasetCpuName)
      ...
      Utils.saveYml[SpearManParams](params, yaml)
...
class SpearManKernel {
  def runDataframeJob(spark: SparkSession,params: SpearManParams): (Double, Double, Double) = {
    ...
    Utils.saveMatrix(spearManMat, params.saveDataPath, sc)
    if (params.isRaw == "yes") {
